<?php
    class db_connection {
        ...
        public function get_schedules($plan_id) {
            ...
            $binds = [$plan_id];
            $schedules = $this->query($sql, $binds);
        ...
        public function add_plan($plan) {
            $sql = "INSERT INTO `plans` (`title`, `description`, 
                `grid_rows`, `grid_cols`) VALUES (?, ?, ?, ?);";
            $stmt = $this->db_connection->prepare($sql);
            $stmt->bind_param("ssii", $plan->title, $plan->description, $plan->grid_rows, $plan->grid_cols);
            $stmt->execute();
            $stmt->close();
        }
